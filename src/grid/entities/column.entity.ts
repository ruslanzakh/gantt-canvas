...
export class ColumnEntity {
	...
	renderDayBackground({ x, today, weekend }: ColumnRender, { monthHeight, dayHeight }: ColumnRenderCommon) {
	...
	renderDayBottomLine({ x }: ColumnRender, { monthHeight, width, dayHeight }: ColumnRenderCommon) {
		const ctx = this.root.ctx;
		ctx.beginPath();
	...
	renderDayStartMonthLine({ x, isStartMonth }: ColumnRender, { monthHeight, dayHeight }: ColumnRenderCommon) {
		const ctx = this.root.ctx;
	...
	renderDayText({ x, title, weekend, month, hour, weekdayTitle }: ColumnRender, { monthHeight, width, dayHeight }: ColumnRenderCommon) {
		const ctx = this.root.ctx;
	...
	renderCol({ x, today, weekend, isStartMonth }: ColumnRender, { monthHeight, dayHeight }: ColumnRenderCommon) {
		const ctx = this.root.ctx;
		ctx.beginPath();
