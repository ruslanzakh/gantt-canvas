...
export class TaskEntity {
	...
	isHover(event: EventOffsets, task: TaskRender) {
		...
		const hover =
			x < offsetX && offsetX < xx && y < offsetY && offsetY < yy;
		if (!hover) return { hover, resize, depFrom };
		if (!noEditable) {
			if (
				this.root.api.taskRenderDepControl &&
	...
	renderItem(task: TaskRender, isTouchAction = false) {
		...
		roundRect(
			...
			top,
			w,
			this.root.api.taskHeight,
			this.root.api.taskRadius,
			fillStyle,
			strokeStyle
		);
	...
	renderRightDep(x: number, y: number) {
		...
		ctx.arc(
			x + this.getDepOffsetX(),
			y,
			this.root.api.taskRenderDepRadius,
			0,
			Math.PI * 2
		);
	...
	renderArrow(id: string, source: TaskRender) {
		const h = this.root.grid.view.rowHeight;
		const task =
			this.root.tasks.service.getRenderedViewTaskById(id) ||
			this.root.tasks.service.getViewTaskById(id);
		if (!task) return;
		const x = source.x + source.w;
		const y = source.y + h / 2;
	...
	renderArrowHead(
		...
		const headlen = 10 * this.root.api.scale;
		...
		if (titleWidth + subtitleWidth < maxWidth) {
			...
			if (subtitle && hover) {
				ctx.fillStyle = convertColor(
					colorSubtitle ?? taskDefaultSubtitleColor
				);
				ctx.fillText(
					subtitle,
					titleX + titleWidth + taskSubtitleOffset,
					top + taskHeight / 2
				);
				if (task.underline)
					renderUnderline(
			...
			ctx.fillStyle = convertColor(
				task.outlineColor ?? taskDefaultOutlineColor
			);
			ctx.textAlign = 'left';
			...
			if (subtitle && hover) {
				ctx.fillStyle = convertColor(
					task.outlineSubtitleColor ?? taskDefaultSubtitleOutlineColor
				);
				ctx.fillText(
					subtitle,
					titleX + titleWidth + taskSubtitleOffset,
	...
	renderResizeControls(task: TaskRender, top: number) {
		if (
			!this.root.api.taskRenderResizeControls ||
		...
		roundRect(
			...
			width,
			height,
			this.root.api.taskRenderResizeControlsRadius,
			color
		);
		roundRect(
			ctx,
	...
	isRenderedControlsHover(
		...
		const endY =
			startY + this.root.api.taskHeight - this.root.api.taskPadding * 2;
	...
	getTaskFillStyle(task: TaskRender): string {
		...
		if (hover || hoverConnection) {
			return convertColor(
				backgroundHover ?? this.root.api.taskDefaultHoverBackground
			);
		}
	...
	getTaskStrokeStyle(task: TaskRender): string | undefined {
		...
		const {
			taskErrorStrokeColor,
			taskDefaultStrokeColor,
			taskDefaultHoverStrokeColor,
		} = this.root.api;
		...
		if (hover || hoverConnection) {
			return convertOptionalColor(
				strokeHover ?? taskDefaultHoverStrokeColor
			);
		}
	...
	getTitleColor(task: TaskRender): string {
		...
		if (hover || hoverConnection) {
			return convertColor(
				colorHover ?? this.root.api.taskDefaultHoverColor
			);
		}
